...
#include <tcb/ranges/view/bom.hpp>
...
TEST_CASE("Byte order mark is prepended correctly", "[bom]")
{
    SECTION("...for UTF-8") {
        ...
        const std::string test = tcb::ranges::view::add_bom(str);
        REQUIRE(test == u8"\ufeff" + str);
    }

    SECTION("...for UTF-16") {
        const std::u16string str = u"" TEST_STRING;
        const std::u16string test = tcb::ranges::view::add_bom(str);
        REQUIRE(test == u"\ufeff" + str);
    }

    SECTION("...for UTF-32") {
        const std::u32string str = U"" TEST_STRING;
        const std::u32string test = tcb::ranges::view::add_bom(str);
        REQUIRE(test == U"\ufeff" + str);
    }

    SECTION("...for wchar_t") {
        const std::wstring str = L"" TEST_STRING;
        const std::wstring test = tcb::ranges::view::add_bom(str);
        REQUIRE(test == L"\ufeff" + str);
    }
}
...
TEST_CASE("Byte order marks are correctly identified", "[bom]")
{
    SECTION("...in UTF-8") {
        const std::string str = u8"\uFEFF" TEST_STRING;
        const std::string test = tcb::ranges::view::consume_bom(str);
        REQUIRE(test == u8"" TEST_STRING);
    }

    SECTION("...in UTF-16") {
        const std::u16string str = u"\uFEFF" TEST_STRING;
        const std::u16string test = tcb::ranges::view::consume_bom(str);
        REQUIRE(test == u"" TEST_STRING);
    }

    SECTION("...in UTF-32") {
        const std::u32string str = U"\uFEFF" TEST_STRING;
        const std::u32string test = tcb::ranges::view::consume_bom(str);
        REQUIRE(test == U"" TEST_STRING);
    }
}
...
TEST_CASE("Native endian InputRanges with byte order marks are stripped correctly", "[bom]")
{
    SECTION("...in UTF-8") {
        std::stringstream ss;
        ss << u8"\uFEFF" TEST_STRING;
        const std::string test = tcb::ranges::view::consume_bom(
                tcb::ranges::rng::istream_range<char>(ss));
        REQUIRE(test == u8"" TEST_STRING);
    }
}
...
TEST_CASE("Byte order marks are correctly used", "[bom]")
{
    ...
    SECTION("...with big-endian wide strings") {
        ...
        const std::wstring test = tcb::ranges::view::consume_bom(str);
